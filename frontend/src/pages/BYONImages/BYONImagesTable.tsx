...
export const BYONImagesTable: React.FC<BYONImagesTableProps> = ({ images }) => {
  ...
  const getActiveFilterCount = React.useCallback((data: BYONImagesFilterDataType): number => {
    ...
    if (
      data[BYONImagesToolbarFilterOptions.type] &&
      data[BYONImagesToolbarFilterOptions.type] !== ImageTypeFilter.all
    ) {
      count++;
    }
    if (
      data[BYONImagesToolbarFilterOptions.enabled] &&
      data[BYONImagesToolbarFilterOptions.enabled] !== ImageEnabledFilter.all
    ) {
      count++;
    }
    return count;
  ...
  const getSessionToggleIndex = React.useCallback(
    () => sessionToggleIndexRef.current,
    [sessionToggleIndexRef],
  );
